...
import { PISTA, CORES, rotuloDia } from '@/lib/timeline/layout'
...
/** Enter/Espaço ativam igual a um clique — mesmo padrão de `SCard`/`DarkKpi`/`BarRow`.
 *  `preventDefault` no Espaço pra não rolar a página. */
function teclaAtiva(cb: () => void) {
...
export function PistaCanvas({ layout, nivel, selecionadoId, onSelecionar, handlers, arrastando, largura }: Props) {
  ...
  // Foco por teclado (Tab) — separado do clique/seleção, só pra dar destaque visual
  // (mesmo "crescer alguns px" que o nó selecionado já tem) a quem está navegando sem mouse.
      ...
      <svg viewBox={`0 0 ${Math.max(1, largura)} ${layout.altura}`} width="100%" height={layout.altura} style={{ display: 'block', fontFamily: FONTE }}>
        ...
        {layout.marcadores.map(m => {
          ...
          return (
            <g
              ...
              tabIndex={0}
              aria-label={m.label}
              onClick={e => { e.stopPropagation(); onSelecionar(m.id) }}
              onKeyDown={teclaAtiva(() => onSelecionar(m.id))}
            ...
            >
              ...
              <circle cx={m.x} cy={PISTA.Y - PISTA.H / 2 - PISTA.BOLINHA_GAP} r={4.5 + (foco ? 3 : 0)} fill="#fff" stroke="#7C6B78" strokeWidth={2} />
        ...
        {layout.toques.map(t => (
          <Toque
            ...
            onFoco={foco => setFocoId(foco ? t.id : null)}
          />
        ))}
        ...
        {layout.nos.map(n => {
          ...
          const foco = focoId === n.id
          const destaque = sel || foco
          const yBorda = bordaPista(n.fileira)
          const rotuloAria = [n.titulo, n.detalhe].filter(Boolean).join(' — ')
          return (
            <g
              key={n.id}
              role="button"
              ...
              onClick={e => { e.stopPropagation(); onSelecionar(n.id) }}
              onKeyDown={teclaAtiva(() => onSelecionar(n.id))}
              onFocus={() => setFocoId(n.id)}
              onBlur={() => setFocoId(null)}
              style={{ cursor: 'pointer' }}
            >
              {!n.terminal && (
              ...
              <g transform={`translate(${n.x},${n.y})`} filter="url(#pista-sombra)">
                <circle r={n.raio + (destaque ? 4 : 0)} fill={n.cor} stroke="#fff" strokeWidth={n.terminal ? 3 : 0} />
...
function Toque({ t, selecionado, focado, onClick, onFoco }: {
  t: ToqueLayout; selecionado: boolean; focado: boolean; onClick: () => void; onFoco: (focado: boolean) => void
}) {
  ...
  const rotuloAria = t.cluster.length > 1
    ? `${t.cluster.length} toques em ${rotuloDia(t.momento.instante)}`
    : `${t.momento.titulo} — ${rotuloDia(t.momento.instante)}`
  return (
    <g
      ...
      onKeyDown={teclaAtiva(onClick)}
      onFocus={() => onFoco(true)}
      onBlur={() => onFoco(false)}
      style={{ cursor: 'pointer' }}
    >
      <circle r={r + (selecionado || focado ? 3 : 0)} fill={t.vazado ? '#fff' : t.cor} stroke={t.contorno ?? (t.vazado ? t.cor : 'none')} strokeWidth={t.contorno ? 3 : t.vazado ? 2 : 0} opacity={reuniao ? 1 : 0.85} />
