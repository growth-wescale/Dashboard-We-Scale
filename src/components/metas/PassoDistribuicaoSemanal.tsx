...
export function PassoDistribuicaoSemanal({
  ...
  onMudarValor: (marca: string, nomePessoa: string, semanaNumero: number, etapa: EtapaMeta, valor: number) => void
}) {
  return (
            ...
            {m.pessoas.map(p => (
              <div key={`${p.nome}-${p.funcao}`} style={{ marginBottom: 16 }}>
                ...
                {ETAPAS_DISTRIBUIVEIS.filter(e => (p.funcao === 'SDR' ? e !== 'Oportunidade COF' && e !== 'Fechamento' : e === 'Oportunidade COF' || e === 'Fechamento')).map(etapa => {
                  ...
                  const itens = distribuicaoSemanal.filter(d => d.marca === m.marca && d.nomePessoa === p.nome && d.etapa === etapa)
