    ...
    <nav style={{
      ...
      <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{ padding: '4px 8px 22px' }}>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 22, letterSpacing: '-0.01em', lineHeight: 1 }}>
            {wordmark}
          </div>
          {brandTag && (
            <div style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', marginTop: 7, color: 'var(--ws-text-on-dark-muted)' }}>
              {brandTag}
            </div>
          )}
        </div>
              ...
              <div key={item.key}>
                <button
                  ...
                  style={{
                    ...
                    fontFamily: 'var(--font-body)', fontWeight: isActive ? 600 : 400, fontSize: 14,
                    padding: '10px 12px', borderRadius: 'var(--radius-sm)', border: 'none',
                    background: isActive ? 'rgba(255,255,255,0.14)' : 'transparent',
                    color: isActive ? '#fff' : 'var(--ws-text-on-dark-muted)',
                ...
                >
                  ...
                  {isActive && !item.subItems && <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--brand-accent)' }} />}
                  {item.badge != null && (
                    <span style={{ fontSize: 11, fontWeight: 600, padding: '2px 7px', borderRadius: 999, background: 'rgba(255,255,255,0.16)' }}>{item.badge}</span>
                  )}
                </button>

                {/* Sub-items */}
                {expanded && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 1, margin: '2px 0 6px', paddingLeft: 10, marginLeft: 20, borderLeft: '1px solid rgba(255,255,255,0.16)' }}>
                    {item.subItems!.map(sub => {
                      ...
                      return (
                        <button key={sub.key} onClick={() => onSelectSub?.(sub.key)} style={{
                          ...
                          textAlign: 'left', cursor: 'pointer', width: '100%',
                          fontFamily: 'var(--font-body)', fontWeight: subActive ? 600 : 400, fontSize: 13,
                          padding: '7px 10px', borderRadius: 'var(--radius-sm)', border: 'none',
                          background: subActive ? 'rgba(255,255,255,0.12)' : 'transparent',
                          color: subActive ? '#fff' : 'var(--ws-text-on-dark-muted)',
        ...
        {footer && (
          <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.14)' }}>
            {footer}
          </div>
        )}
      </div>
